...
namespace Valve.VR.InteractionSystem.Sample
{
    ...
    public class GunMenuController: MonoBehaviour
    {
        ...
        public UnityEvent instructions;
        public UnityEvent quit;
        public UnityEvent hardSetting, mediumSetting, easySetting;
        ...
        private void ShootGun()
        {
            ...
            {
                if (hit.collider.gameObject.CompareTag("PlayGame"))
                {
                    startGame.Invoke();
                }

                if (hit.collider.gameObject.CompareTag("Back"))
                {
                    mainMenu.Invoke();
                }

                if (hit.collider.gameObject.CompareTag("Instructions"))
                {
                    instructions.Invoke();

                }

                if (hit.collider.gameObject.CompareTag("Quit"))
                {
                    quit.Invoke();

                }

                if (hit.collider.gameObject.CompareTag("Hard"))
                {
                    hardSetting.Invoke();
                }

                if (hit.collider.gameObject.CompareTag("Medium"))
                {
                    mediumSetting.Invoke();
                }

                if (hit.collider.gameObject.CompareTag("Easy"))
                {
                    easySetting.Invoke();
                }
            }
